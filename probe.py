...
class Probe: 
    """ Debug tool. View local time series data at pre-defined locations.
        ...
        Usage in __main__:
            path_probe = Pro.Probe(sid, edges, graph, snode=8, opts="path", total_nodes=10)
            ...
            ...
                path_probe.record(
                    ...
                    edge_data = {'flow': edges.flow, 'diams': edges.diams}
                )
                path_probe.plot_time_series_data(sid, edges, graph)
            ...

        TODO: 'live' probes /hotspots on the network 
    """
    ...
    def __init__(self, sid, edges, graph, snode=20, opts=None, total_nodes=None):
        ...
        # Nodes on a directed path or a square grid
        if opts == "path": 
    ...
    def get_left_to_right_path(self, sid, edges: Edges, graph: Graph):
        """ Gets a random path from inlet to outlet that moves +ve horizontal direction
        ...
        if self.start_node > sid.n or self.start_node < 0:
        ...
        for xt in x_targets:
            for yt in y_targets:
                ...
                if (u, v) in edges.edge_list:
                    ...
                    print(f"Edge between {u} and {v} not found. Skipping.")
                    continue 
                grid_nodes.append(u)
