...
class Data():
    ...
    def check_mass_balance(self, sid, inc, edges, vols, cb, cc, cd,
        ...
        Physical model
        --------------
        Dissolution:    A(s) + B(aq) → C(aq) + products
                        rate ∝ Da/(1+G·d)

        Precipitation:  C(aq) + D(aq) → E(s)
                        P(D0) = Da·K·d·(D0/Kp)
                                / [1 + G·K·d·(D0/Kp)]

        Here ``D0`` is the upstream concentration of species D on an edge.
        This is the same concentration-aware transverse-hindering law used by
        ...
        (3) D flux vs precipitation:
            The exact global D consumption, ``D_in - D_out``, is compared both
            with the global C/B stoichiometric balance and with an edgewise
            reconstruction using the same D0-dependent coefficient, analytical
            C solution, resonance limit, and finite-D cap as the precipitation
            solver.  C and D are consumed 1-for-1 in precipitation, so
            E formation rate = (Γ/Da) × (D_in − D_out).
        ...
        # Edgewise reconstruction using exactly the same local constitutive
        # law as solve_precipitation_nr9_vxx_d0_hindering and
        # solve_dp_vol_d0_hindering.  D0 is frozen at the upstream edge value,
        # but it enters both the pseudo-first-order rate and its transverse
        # diffusive hindering:
        #
        #   P(D0) = Da*K*d*(D0/Kp) / [1 + G*K*d*(D0/Kp)].
        #
        # The C outlet solution includes C generated by dissolution along the
        # edge, so this replaces the previous cc_up*(1-exp_p) approximation.
        if sid.Kp <= 0:
            raise ValueError("sid.Kp must be positive")

        d = np.asarray(edges.diams, dtype=float)
        length = np.asarray(edges.lens, dtype=float)
        alpha_b = np.asarray(edges.alpha_b, dtype=float)
        alpha_c = np.asarray(edges.alpha_c, dtype=float)

        # Dissolution coefficient r and edge conversion, identical to the B
        # transport law used above.
        b_rate = alpha_b * sid.Da * d / (1.0 + sid.G * d)
        residence = length / q_safe
        exp_b_edge = np.exp(np.clip(-np.abs(b_rate * residence), -700.0, 700.0))
        cb_edge_out = cb_up * exp_b_edge

        # Concentration-aware precipitation coefficient and alpha_c-scaled
        # sink s.  Negative trial concentrations are not expected here, but
        # clipping mirrors the nonlinear solver's rate evaluation.
        ...
        sink_rate = alpha_c * p_rate
        exp_p = np.exp(np.clip(-np.abs(sink_rate * residence), -700.0, 700.0))

        # Stable analytical solution for C:
        #   C1 = C0 exp(-s x)
        #        + A [exp(-r x)-exp(-s x)]/(s-r),
        # with the Taylor limit used near s=r.
        ...
        quotient[regular] = (
            exp_b_edge[regular] - exp_p[regular]
        ) / denominator[regular]

        yr = y[near_resonance]
        xr = residence[near_resonance]
        er = exp_b_edge[near_resonance]
        ...
        source_amplitude = b_rate * cb_up
        cc_edge_out = cc_up * exp_p + source_amplitude * quotient
        cc_edge_out = np.where(np.isfinite(cc_edge_out), cc_edge_out, cc_up)
        cb_edge_out = np.where(np.isfinite(cb_edge_out), cb_edge_out, cb_up)

        # Same stoichiometric finite-D cap as the nonlinear solver.
        d_consumed_edge = (cb_up - cb_edge_out) + (cc_up - cc_edge_out)
        d_consumed_edge = np.minimum(
            np.maximum(d_consumed_edge, 0.0),
            cd_rate,
        )
        d_consumed_edge[~flowing] = 0.0

        # Reconstructed network precipitation sink in concentration-flow units.
        prec_flux = np.sum(abs_q * d_consumed_edge)
        ...
        if verbose:
            print(
                ...
                f"     edge-reconstructed D sink (D0 hindering):   {prec_flux:.4e}\n"
                f"     D−edge-sink residual / rel:                  {D_residual:+.4e} / {D_rel:+.4e}\n"
        ...
        return {
            ...
            'prec_flux':               prec_flux,
            'prec_flux_d0_reconstructed': prec_flux,
