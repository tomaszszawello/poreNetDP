...
def fix_merging(sid: SimInputData, inc: Incidence, graph: Graph, \
    ...
    for edge in merge_fix_edge:
        try:
            ...
            raise ValueError("Merging problem (the worst kind - try simulating again)")
...
def solve_merging_vols(sid: SimInputData, inc: Incidence, graph: Graph, vols: Volumes, \
    ...
    while np.sum(1 * (vols.triangles @ (vols.vol_a + vols.vol_e == 0))):
        ...
        merge_edges = []
        merge_triangles = []
        merge_matrix = spr.diags(1 * (vols.vol_a + vols.vol_e == 0)) @ vols.tri_contact.T
        if merge_matrix.sum():
        ...
        for tr_i, edge_list_tri in enumerate(merge_edges):
            ...
            # NEW: filter edges with incidence row having at least two nonzeros
            candidate_edges = [
                e for e in edge_list_tri
                if (e not in skip_list)
                and (edges.diams[e] > 0)
                and (inc.incidence[e].getnnz() >= 2)   # <<< NEW
            ]
            if len(candidate_edges) < 1:
                # prevent re-queuing this triangle forever
                # optional: clear the working triangle column so we progress
                for e in edge_list_tri:
                    vols.triangles[e, merge_triangles[tr_i]] = 0
                    vols.tri_contact[e, merge_triangles[tr_i]] = 0
                print('skip: not enough valid edges on triangle', merge_triangles[tr_i])
                continue
            flag = 0
            ...
            nodes_merge = inc.incidence[merge_i].nonzero()[1]
            if nodes_merge.size != 2:
                # stop looping on this triangle this pass
                for e in edge_list_tri:
                    vols.triangles[e, merge_triangles[tr_i]] = 0
                # also drop from contact memory to avoid future picks
                vols.tri_contact[merge_i, :] = 0
                print('merge_i has invalid incidence row:', merge_i, nodes_merge.size)
                continue
            n1, n2 = nodes_merge

            nodes_zero = inc.incidence[zero_i].nonzero()[1]
            if nodes_zero.size != 2:
                for e in edge_list_tri:
                    vols.triangles[e, merge_triangles[tr_i]] = 0
                vols.tri_contact[zero_i, :] = 0
                print('zero_i has invalid incidence row:', zero_i, nodes_zero.size)
                continue
            n3, n4 = nodes_zero
            ...
            #     print('too far away')
            #     continue
            if np.abs(pos[merge_node][1] - pos[zero_node][1]) > 5:
                for edge in edge_list_tri:
                    vols.triangles[edge, merge_triangles[tr_i]] = 0
                    vols.tri_contact[edge, merge_triangles[tr_i]] = 0
                print('too far away')
                continue
                #raise ValueError("Wrong merging!")
            merge_nodes_flat = np.reshape(merged_nodes, 2 * len(merged_nodes))
        ...
        if len(merged) > 0:
            ...
            vols.tri_contact = (diag_edges @ vols.tri_contact).tocsr()
            diag_edges *= spr.diags(1 * (sign_fix != 0))
            edges.inlet *= (sign_fix != 0)
            edges.outlet *= (sign_fix != 0)
            inc.incidence = spr.diags(sign_fix) @ inc.incidence
...
def fix_lonely_triangles(sid: SimInputData, inc: Incidence, graph: Graph, \
    ...
    """
    lonely_triangles =  np.where((np.array(np.sum(vols.triangles.T, axis = 1))[:, 0] < 1) * (vols.vol_a + vols.vol_e > 0))[0]
...
def extract_tail_head(B, strict=False):
    B = B.tocsr(copy=True)
    B.sum_duplicates()
    B.eliminate_zeros()

    E, N = B.shape
    tail = np.full(E, -1, dtype=np.int64)
    head = np.full(E, -1, dtype=np.int64)

    indptr, indices, data = B.indptr, B.indices, B.data
    for e in range(E):
        s, t = indptr[e], indptr[e+1]
        cols = indices[s:t]; vals = data[s:t]
        neg = np.flatnonzero(vals == -1)
        pos = np.flatnonzero(vals == +1)
        if neg.size == 1 and pos.size == 1:
            tail[e] = cols[neg[0]]
            head[e] = cols[pos[0]]
        elif strict:
            raise ValueError(f"Bad incidence row {e}: cols={cols}, vals={vals}")
    return tail, head
