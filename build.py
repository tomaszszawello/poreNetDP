...
def build() -> tuple[SimInputData, In.Incidence, Ne.Graph, In.Edges, Data]:
...
def loop_build(sid: SimInputData) -> tuple[In.Incidence, Ne.Graph, In.Edges, Data]:
    ''' Initialize main classes used in simulation based on config file.
    ...
    # but in an additional folder named template, save new config there
        ...
        print ('load 2')
        sid2, graph, inc, edges \
            = Sv.load(sid.load_name+'/template.dill')
        sid.Q_in = sid.qin * 2 * len(graph.in_nodes)
        sid.n = sid2.n
        sid.nsq = sid2.nsq
        sid.ne = sid2.ne
