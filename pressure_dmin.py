...
def solve_flow(sid: SimInputData, inc: Incidence, graph: Graph, edges: Edges, \
    ...
    edges_perm = (edges.diams ** 5 / edges.diams_initial + (edges.diams_initial - edges.diams) / edges.diams_initial * (edges.diams - sid.dmin) ** 4) / edges.lens
    #edges_perm = ((edges.diams - sid.dmin) * edges.diams ** 4 / edges.diams_initial + (edges.diams_initial - edges.diams + sid.dmin) / edges.diams_initial * (edges.diams - sid.dmin) ** 4) / edges.lens
    p_matrix = inc.incidence.T @ spr.diags(edges_perm) \
        @ inc.incidence
    ...
    q_in = np.abs(np.sum(edges_perm * (inc.inlet \
        @ pressure)))
    pressure *= sid.Q_in / q_in
    # update flow
    edges.flow = edges_perm * (inc.incidence @ pressure)
