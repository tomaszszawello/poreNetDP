...
def update_diameters(sid: SimInputData, inc: Incidence, edges: Edges, graph: Graph, \
    ...
    dt_next : float
        new timestep
    """
    change, change_min = solve_d(sid, inc, edges, cb, cc)
    breakthrough = False
    if sid.include_adt:
        ...
        dt_next = sid.dt
    diams_min = edges.diams_min - change_min * dt_next
    ...
    if sid.cut == 'edges':
        ...
        diams_new = diams_new * (diams_min > 0)
        if np.sum(diams_new == 0) != np.sum(edges.diams == 0):
            print("Edges cut")
    ...
    edges.diams_min = diams_min * (diams_min > 0)
...
def solve_d(sid: SimInputData, inc: Incidence, edges: Edges, cb: np.ndarray, cc: np.ndarray) \
    ...
    change_min = -cb_in * cc_in
    return change, change_min
