...
def load_all(name):
    ...
    return graph, incidence, diams, lens, flows, in_edges, pressure, inlet


@njit
def track_particle2(neigh_inc, tot_flow, inlet_flow, edge_list, out_nodes, tot_time, exp, pressure, max_neighbors):
    time = 0.0
    conc = 1.0
...
@njit(parallel=True)
def parallel_tracking(neigh_inc, tot_flow, inlet_flow, edge_list, out_nodes, tot_time, exp, pressure, max_neighbors, n_part):
    breakthrough_times = np.zeros(n_part)
    concentrations = np.zeros(n_part)
    for i in range(n_part):
        time, conc = track_particle2(neigh_inc, tot_flow, inlet_flow, edge_list, out_nodes, tot_time, exp, pressure, max_neighbors)
        breakthrough_times[i] = time
        concentrations[i] = conc
    
    return breakthrough_times, concentrations

def track(graph, incidence, inlet, flow, lens, diams, pressure, G, Da, n_part) -> None:
    ...
    breakthrough_times, concentrations = parallel_tracking(neigh_inc.toarray(), tot_flow, inlet_flow, edge_list, graph.out_nodes, tot_time, exp, pressure, max_neighbors, n_part)
    return breakthrough_times, concentrations
...
def plot_tracking_standard(tracking, concentration_tracking, dirname, networks, nbins) -> None:
    ...
    #ax.set_title('BTC')
    ax.set_xlabel('arrival time / peak arrival time')
    #ax2.set_xlabel('time')
    ...
    ax.loglog(bx11, pdf, "o", alpha = 1, markersize=12, label = networks[0], color = colors[0])
    bx11, pdf = create_pdf(tracking[-1], nbins, x = bx1)
    ax.loglog(bx11, pdf, "o", alpha = 1, markersize=12, label = networks[-1], color = colors[-1])
    plt.plot(bx11, bx11 ** (-3))
    plt.plot(bx11, 10 * bx11 ** (-5))
    #legend = ax3.legend(loc='center right', bbox_to_anchor=(1.05, 0.5), prop={'size': 40}, frameon=False, handlelength = 0.2, borderpad = 0, handletextpad = 0.4)
    ...
    plt.close()
#%%
dirname = 'tracking/Pe100.00Da0.10/3/'
networks = ['0.0', '1.0']
tracking_list = []
c_tracking_list = []
nbins = 100
...
for net in networks:
    ...
    graph, incidence, diams, lens, flow, in_edges, pressure, inlet = load_all(name)
    tracking, c_tracking = track(graph, incidence, inlet, flow, lens, diams, pressure, G, Da, n_part)

    bx1 = create_bins(tracking, nbins)
    bx11, pdf = create_pdf(tracking, nbins, x = bx1)
    peak_t = bx1[np.where(pdf == pdf.max())]
    print(peak_t)
    tracking_list.append(tracking / peak_t)
...
#%%    
nbins = 100
plot_tracking_standard(tracking_list, c_tracking_list, dirname, networks, nbins)
#%%
import os
for i in range(1, 31):
    print(i)
